          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '2px solid rgba(0,0,0,0.1)' }}>
                <th className="text-left text-[10px] sm:text-xs font-semibold uppercase tracking-wider pb-2 sm:pb-3" style={{ color: '#6B7280' }}>Ingredient</th>
                <th className="text-right text-[10px] sm:text-xs font-semibold uppercase tracking-wider pb-2 sm:pb-3 w-[80px] sm:w-[100px]" style={{ color: '#6B7280' }}>Amount</th>
                <th className="text-right text-[10px] sm:text-xs font-semibold uppercase tracking-wider pb-2 sm:pb-3" style={{ color: '#6B7280' }}>Benefit</th>
              </tr>
            </thead>
            <tbody>
              {ingredients.map((item) => (
                <tr key={item.name} style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                  ...
                  <td className="text-sm sm:text-base font-bold text-right py-2.5 sm:py-3 tabular-nums" style={{ color: '#0D8A72' }}>{item.amount}</td>
        ...
        <div className="flex gap-2 overflow-x-auto whitespace-nowrap pb-1">
          {['100% Vegan', '0g Sugar', 'No Artificial Anything', '10 Cal'].map((label) => (
            <span
              ...
              style={{ backgroundColor: '#0D7C6B', color: 'white' }}
            >
              {label}
            </span>
          ))}
